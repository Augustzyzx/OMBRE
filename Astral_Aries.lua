...
if isfolder("Astralise hub") then
    delfolder("Astralise hub")
end
...
function Library:load_flags()
    if not isfile(`Astralise hub/{game.GameId}.lua`) then Library.save_flags() return end

    local flags = readfile(`Astralise hub/{game.GameId}.lua`)
