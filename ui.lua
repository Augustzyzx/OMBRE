...
ToggleButton.TextColor3 = Color3.fromRGB(255, 0, 127)
...
function library:new(props)
	...
	local outline = utility.new(
		"Frame",
		{
			...
			BorderColor3 = Color3.fromRGB(255, 0, 127),
			BorderSizePixel = 1,
...
function library:loader(props)
	...
	local makebutton = function(name,parent)
		...
		utility.new(
			"UIGradient",
			{
				Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(199, 191, 204)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(255, 0, 127))},
				Rotation = 90,
				Parent = button_color
			}
		)
		...
		ddoptionbutton.MouseButton1Down:Connect(function()
			...
			for z,x in pairs(dropdown.titles) do
				if x.TextColor3 == self.library.theme.accent then
					x.TextColor3 = Color3.fromRGB(255, 0, 127)
				end
			end
			dropdown.current = v
	...
	dropdownbutton.MouseButton1Down:Connect(function()
		...
		for i,v in pairs(dropdown.titles) do
			if v.Text == dropdown.current then
				...
				v.TextColor3 = Color3.fromRGB(255, 0, 127)
			end
		end
...
function sections:multibox(props)
	...
	for i,v in pairs(options) do
		...
		ddoptionbutton.MouseButton1Down:Connect(function()
			...
			if find == nil then
				...
				value.Text = str
				ddoptiontitle.TextColor3 = Color3.fromRGB(255, 0, 127)
				multibox.callback(multibox.current)
			end
		end)
	end
	...
	dropdownbutton.MouseButton1Down:Connect(function()
		...
		for i,v in pairs(multibox.titles) do
			if v.TextColor3 ~= Color3.fromRGB(255, 0, 127) then
...
function sections:colorpicker(props)
	...
	local def = props.def or props.Def or props.default or props.Default or Color3.fromRGB(255, 0, 127)
	local callback = props.callback or props.callBack or props.CallBack or props.Callback or function()end
	--
	...
	local function textbox(parent,size,position)
		...
		local tbox = utility.new(
			"TextBox",
			{
				...
				Size = UDim2.new(1,0,1,0),
				Position = UDim2.new(0.5,0,0,0),
				PlaceholderColor3 = Color3.fromRGB(255, 0, 127),
				PlaceholderText = "",
				Text = "",
				TextColor3 = Color3.fromRGB(255, 0, 127),
				TextSize = self.library.textsize,
		...
		createdbutton.MouseButton1Down:Connect(function()
			for i,v in pairs(createdbuttons) do
				if v ~= createdb then
					...
					v.title.TextColor3 = Color3.fromRGB(255, 0, 127)
